...
import copy
import cv2
from matplotlib.pyplot import axis # You may have to "pip install opencv-contrib-python" to install this
...
def expand_nr(img_mask, cur_coord, coordinates_in_blob):
  ...
  while len(coordinate_list) > 0:
    ...
    if((cur_coordinate[0] < 0 or cur_coordinate[1] < 0) or (cur_coordinate[0] >= img_mask_height or cur_coordinate[1] >= img_mask_width)):
      continue
...
def get_blob_centroids(blobs_list):
  ...
  for x in blobs_list:
    if(len(x) > 1000):
      object_positions_list.append(np.mean(x, axis=0))
...
def main():
  ...
  add_color_range_to_detect([4,0,107],[64,80,255])
  add_color_range_to_detect([38,1,0],[255,149,80])
  add_color_range_to_detect([3,60,0],[98,225,63])
  add_color_range_to_detect([0,181,235],[48,247,255])
